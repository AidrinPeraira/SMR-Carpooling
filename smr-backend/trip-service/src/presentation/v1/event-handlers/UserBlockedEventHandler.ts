...
import { DriverStatus, ILogger, UserBlockedEvent } from "@sharemyride/shared";
...
export class UserBlockedEventHandler implements IEventHandler<UserBlockedEvent> {
  ...
  async handle(event: UserBlockedEvent): Promise<void> {
    ...
    this._logger.info(
      `Processing ADMIN_USER_BLOCKED event for passenger ${userId}`,
    );
    await this._blockPassengerUseCase.execute({ passengerId: userId });

    if (isDriver) {
      this._logger.info(
        `Processing ADMIN_USER_BLOCKED event for driver ${userId}`,
      );
