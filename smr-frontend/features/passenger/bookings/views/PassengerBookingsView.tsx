...
import { getPassengerBookingsRequest } from "../api/getPassengerBookingsRequest";
import { PassengerBookingsHeader } from "../components/PassengerBookingsHeader";
import { PassengerBookingsCard } from "../components/PassengerBookingsCard";
...
export function PassengerBookingsView() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["passengerBookings", statusFilter, page, limit],
    queryFn: () =>
      getPassengerBookingsRequest({
        ...
        <div className="rounded-xl border border-border-subtle bg-surface-card p-12 text-center my-8 backdrop-blur-sm">
          ...
          <p className="text-sm text-content-secondary mt-1 max-w-sm mx-auto">
            {statusFilter !== "all"
              ...
              : "You haven't booked any carpool rides yet."}
          </p>
        </div>
      )}
