...
export function DriverBookingsCard({ booking }: DriverBookingsCardProps) {
  ...
  const renderStatusTag = (status: string) => {
    ...
    switch (s) {
      case "confirmed":
        return (
          <Tag variant="accent" className="bg-success-surface text-success-content border-success-border">
            CONFIRMED
          </Tag>
        );
      case "requested":
        return (
          <Tag variant="muted" className="bg-warning-surface text-warning-content border-warning-border">
            REQUESTED
          </Tag>
        );
      case "payment_pending":
        return (
          <Tag variant="muted" className="bg-accent/15 text-accent border-accent/30">
            PAYMENT PENDING
          </Tag>
        );
      case "rejected":
      case "cancelled":
        return (
          <Tag variant="muted" className="bg-error-surface text-error-content border-error-border">
            {s.toUpperCase()}
          </Tag>
        );
      default:
        ...
        <div className="lg:w-1/4 space-y-2">
          ...
          <div className="text-xs text-content-secondary space-y-1">
            ...
            <p className="flex items-center gap-1.5">
              ...
              Vehicle: <span className="font-semibold text-content-primary">{booking.trip_vehicle}</span>
            </p>
          </div>
        </div>
        ...
        <div className="lg:flex-1 border-t lg:border-t-0 lg:border-l border-border-subtle pt-4 lg:pt-0 lg:pl-6">
          <div className="flex items-start gap-3">
            <div className="flex flex-col items-center mt-1">
              ...
              <div className="w-3 h-3 rounded-full bg-error-content ring-4 ring-error-content/15" />
            </div>
            <div className="flex-1 space-y-3 text-sm">
              <div>
                ...
                <p className="font-bold text-content-primary">{booking.pickup_point_name}</p>
              ...
              <div>
                ...
                <p className="font-bold text-content-primary">{booking.drop_off_point_name}</p>
        ...
        <div className="lg:w-1/4 border-t lg:border-t-0 lg:border-l border-border-subtle pt-4 lg:pt-0 lg:pl-6 flex flex-col justify-between gap-3">
          <div className="grid grid-cols-3 gap-2 text-center bg-surface-muted rounded-lg p-3 text-xs">
            <div>
              ...
              <span className="font-semibold text-content-primary">{booking.booking_distance} km</span>
            </div>
            <div>
              <span className="text-content-secondary block">Seats</span>
              <span className="font-semibold text-content-primary">{booking.seat_count} seat(s)</span>
            </div>
            <div>
          ...
          <Link href={`/driver/bookings/${booking.booking_id}`} className="w-full">
